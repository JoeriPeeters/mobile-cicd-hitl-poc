...
```
 ...
 🤖 3b. (optional) advisory agents              │   findings posted on the PR, not blocking
        ├─ Visual fidelity agent compares the   │   screenshot vs the spec's Figma frame
        │   screenshot vs the linked Figma frame │   (docs/visual-fidelity-agent.md)
...
| 🤖 **Advisory agents** | review the diff / screenshots (visual, security) — advisory, not blocking | [visual fidelity agent](visual-fidelity-agent.md); OWASP agent (reference repo) |
